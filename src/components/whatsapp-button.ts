...
export function mountStickyWhatsApp(): void {
  const root = document.getElementById('whatsapp-sticky');
  if (!root) return;
  ...
  root.innerHTML = `
    <a
      ...
      data-whatsapp-sticky
      class="fixed bottom-4 right-4 z-50 flex items-center gap-2 rounded-full bg-bronze-500 px-5 py-3 text-sm font-semibold text-graphite-950 shadow-lg shadow-black/40 transition duration-200 hover:scale-105 hover:bg-bronze-400 md:hidden"
      aria-label="Falar pelo WhatsApp com o escritório"
    >
  ...
  root.querySelector('[data-whatsapp-sticky]')?.addEventListener('click', () => {
